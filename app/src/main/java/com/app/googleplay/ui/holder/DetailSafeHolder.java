...
public class DetailSafeHolder extends BaseHolder<AppInfo> {
    ...
    private void toggle() {
        ValueAnimator animator = null;//通过属性动画动态改变llDesRoot高度
        if (isOpen) {
            // 关闭
            isOpen = false;
            // 属性动画
    ...
    public void refreshData(AppInfo data) {
        ...
        for (int i = 0; i < 4; i++) {
            if (i < safe.size()) {
                ...
                mBitmapUtils.display(mSafeIcons[i], HttpHelper.URL + safeInfo.safeUrl);
                System.out.println("图片URL=" + safeInfo.safeUrl);
                // 安全描述文字
                mSafeDes[i].setText(safeInfo.safeDes);
                System.out.println("图片文字=" + safeInfo.safeDes);
                // 安全描述图片
                mBitmapUtils.display(mDesIcons[i], HttpHelper.URL + safeInfo.safeDesUrl);
                System.out.println("图片描述URL=" + safeInfo.safeDesUrl);
            } else {
                // 剩下不应该显示的图片
